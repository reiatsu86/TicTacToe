...
function placeXOrO(squareNumber) {
    ...
    if (!selectedSquares.some(element => element.includes(squareNumber))) {
        ...
        //If active played may only be 'X' or 'O' so, if not 'X' it must be 'O'    
        }else {
        ...
        // Fix: check the win condition before the computer plays
        checkWinConditions();
...
function checkWinConditions() {
    ...
    // o 0,1,2 condition.
    if (arrayIncludes ('0O', '1O', '2O')) {drawWinLine(50, 100, 550, 100);}
    // o 3,4,5 condition.
...
function audio(audioURL) {
    //we create a new audio object and we pass the path a parameter.
...
function resetGame() {
    ...
    for (let i = 0; i < 9; i++) {
        ...
        let square = document.getElementById(String(i)); // fix: string to String
        //This removes our element backgroundsImage.
